	...
	for (int i = 0; i < noOfVertices; i++)									//for each vertex generate between 1 and 4 edges
	{
		...
		for (int j = 0; j < currEdges; j++)									//for each edge
		{
			...
			do
			{
				...
				{
					fp >> pEdge[j].source >> pEdge[j].destination >> pEdge[j].weight;
...
{
	for (unsigned int i = 1; i <= visited.size(); i++)						//search the visited vector for the node value
