...
void printGraph(FILE *out, graph *G)
{
	...
	//fscanf(out, "%d", &V);
	//fscanf(out, "%d", &E);
	fprintf(out, "%d ", G->noOfVertices );
	fprintf(out, "%d \n", G->noOfEdges );

			// allocate memory for the edges
	for (int i = 0; i<G->noOfEdges; i++)										// set the edges
	{
	    fprintf(out, "%d ", G->pEdge[i].source);
	    fprintf(out, "%d ", G->pEdge[i].destination);
		fprintf(out, "%d \n", G->pEdge[i].weight);
	}

	fclose(out);													// return the graph
}
...
void printGraph(graph *G)
{
	...
	for (int i = 0; i<G->noOfEdges; i++)										// set the edges
	{
	    printf("%d ", G->pEdge[i].source);
	    printf("%d ", G->pEdge[i].destination);
		printf("%d \n", G->pEdge[i].weight);
	}
    system("PAUSE");
													// return the graph
...
void printGraph_ceva(graph *G)
{
    int counter=0;

	for (int i = 0; i<G->noOfVertices; i++)										// set the edges
	{
	    for( int j=0; j<G->noOfVertices; j++){
                if(G->pEdge[i].destination == G->pEdge[j].source){
                    printf("%d ", G->pEdge[counter].weight);
                    counter++;
                }else{
                    printf("0 ");
                }
